...
\begin{longtblr}[
  ...
  colspec = {X[0,l] >{\bfseries}X[0.5,l] c Q[m,c] X[l,t]},
  rowhead = 1,
  row{1}  = {gray9}}
    \toprule
        & \bfseries Name
    ...
    \\\midrule
        ...
        & $h_{ij}/h_{iA}$
        & $\displaystyle\sum^\infty_{n=1}\mathds P_i(H_A=n)$
    ...
    \\\midrule
        ...
        & $V_j/V_A$
        & $\displaystyle\sum^\infty_{n=0}\mathds{1}_{\{X_n=i\}}$
        & Number of visits to state $j$/set $A$.
    \\\midrule
        & \boldred{E}xpected number of \boldred{v}isits
        & $\gamma_j^i$
        & $\mathds E_i\left[\displaystyle\sum^{T_j-1}_{n=0}\mathds1(X_n=j)\right]$
